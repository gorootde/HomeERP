...
<div class="flex flex-wrap gap-1.5">
  {#each tags as tag}
    <span class="inline-flex items-center gap-1 px-2 py-0.5 bg-indigo-100 text-indigo-700 rounded-full text-xs font-medium">
      {tag.name}
      {#if !readonly}
        <button onclick={() => onremove?.(tag.name)} class="hover:text-indigo-900">
          <X size={12} />
        </button>
      {/if}
    </span>
  ...
  {#if !readonly}
    <div class="flex items-center gap-1">
      <input bind:value={input} onkeydown={keydown}
        ...
        class="text-xs border border-gray-300 rounded-full px-2 py-0.5 w-28 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
      <button onclick={add} class="text-indigo-600 hover:text-indigo-800">
        <Plus size={14} />
      </button>
    </div>
  {/if}
