...
from ..helpers import add_tag, get_or_404, raise_if_exists, remove_tag
from ..models import EanCode, Product, ProductUnitConversion
from ..schemas import (
    EanCodeCreate,
    EanCodeRead,
    ProductCreate,
...
def _save_product_image(product: Product, data: bytes, suffix: str, db: Session) -> Product:
    """Write ``data`` to a fresh file, drop the old image (if any), and save."""
...
def create_product(data: ProductCreate, db: Session = Depends(get_db)):
    ...
    for code in data.ean_codes:
        raise_if_exists(
            db, EanCode, f"EAN {code} is already assigned to another product", code=code
        )
...
def get_product(product_id: int, db: Session = Depends(get_db)):
    return get_or_404(db, Product, product_id, "Product not found")
...
def add_ean(product_id: int, data: EanCodeCreate, db: Session = Depends(get_db)):
    ...
    raise_if_exists(
        db, EanCode, f"EAN {data.code} is already assigned to another product", code=data.code
    )
...
async def upload_image(product_id: int, file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    return _save_product_image(product, data, suffix, db)
...
def add_tag_to_product(product_id: int, data: TagCreate, db: Session = Depends(get_db)):
    product = get_or_404(db, Product, product_id, "Product not found")
    return add_tag(db, product, data.name)
...
def remove_tag_from_product(product_id: int, tag_name: str, db: Session = Depends(get_db)):
    product = get_or_404(db, Product, product_id, "Product not found")
    remove_tag(db, product, tag_name)
...
def image_from_url(product_id: int, body: _ImageUrlBody, db: Session = Depends(get_db)):
    ...
    return _save_product_image(product, image_data, ext, db)
...
def add_product_unit_conversion(product_id: int, data: ProductUnitConversionCreate, db: Session = Depends(get_db)):
    get_or_404(db, Product, product_id, "Product not found")
    raise_if_exists(
        db, ProductUnitConversion,
        f"Einheit '{data.unit_name}' ist für dieses Produkt bereits definiert",
        product_id=product_id, unit_name=data.unit_name,
    )
